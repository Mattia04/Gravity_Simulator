...
class Vector3D:
    ...
    def __str__(self) -> str:
        return f"Coordinate cartesiane \t(x: {self.x:+.2e}, \ty: {self.y:+.2e},\tz: {self.z:+.2e})\n"\
            ...
            + f"Coordinate colindriche \t(ρ: {self.rho():+.2e}, \tφ: {self.azm_angle():+.2e},\tz: {self.z:+.2e})\n"
    ...
    def __rmul__(self, other : int|float) -> float:#note that in rmul self is at the right and other at the left
    ...
    #Return the cilindrical position as a tuple
    def get_ciln_coord(self) -> Tuple[float]:
    ...
    def azm_angle(self) -> float:
        if self.mod() == 0:
            raise Exception("The vector is a zero: it's not possible to define an azimutal angle\n")
        if self.x == 0:
            return 0
