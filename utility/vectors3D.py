...
class Vector3D:
    ...
    def __str__(self) -> str:
        return f"Coordinate cartesiane \t(x: {self.x:+.2e}, \ty: {self.y:+.2e},\tz: {self.z:+.2e})\n"\
            + f"Coordinate polari      \t(r: {self.mod():+.2e}, \tɸ: {self.azimuth_angle():+.2e},\tθ: {self.polar_angle():+.2e})\n"\
            + f"Coordinate cilindriche \t(ρ: {self.rho():+.2e}, \tφ: {self.azimuth_angle():+.2e},\tz: {self.z:+.2e})\n"
    ...
    def get_polar_coord(self) -> Tuple[float]:
        return self.mod(), self.azimuth_angle(), self.polar_angle()

    #Return the cylindrical position as a tuple
    def get_cylindrical_coord(self) -> Tuple[float]:
        return Vector2D(self.x, self.y).mod(), self.azimuth_angle(), self.z
